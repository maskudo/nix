...
local function delete_mark(mark, bufnr)
  ...
  vim.fn.sign_unplace(
    sign_group_name,
    { buffer = bufnr, id = buffer_marks[mark].id }
  )
...
function Marks.BufWinEnterHandler(args)
  ...
  for _, data in ipairs(vim.fn.getmarklist()) do
    ...
    if
      mark_buf == bufnr
      and is_uppercase_mark(mark)
      and (not cached_mark or mark_line ~= cached_mark.line)
    then
...
vim.api.nvim_create_autocmd("BufWinEnter", {
  group = vim.api.nvim_create_augroup("maskudo/marks", { clear = true }),
  callback = function(ev)
    Marks.BufWinEnterHandler(ev)
  end,
})
